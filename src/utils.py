...
from custom_datasets.celbahq import CelebaDataset, CelebaDatasetPath
from custom_datasets import smallimagenet
from custom_datasets.imagenet_hierarchy import ImageNetDataSet
...
def get_model_info(args):
    ...
    if args.net == 'imagenet' or args.net == 'imagenet_hierarchy':
        ...
        get_debug_info(msg="Info: Take default net!")        
...
def load_model(args):
    ...
    if args.net == 'mnist':
        model = WideResNet(num_classes=settings.MAX_CLASSES_MNIST, block=WideBasic, depth=depth, widen_factor=widen_factor, preprocessing=net_normalization)
        
        ckpt = torch.load(settings.MNIST_CKPT)
        ...
        # cifar 10 modified without activation function
        model = Wide_ResNet(28, 10, 0.3, num_classes=settings.MAX_CLASSES_CIF10, preprocessing=net_normalization)

        # import pdb; pdb.set_trace()
        model_dict = model.state_dict()
        save_model = torch.load(settings.CIF10_M_CKPT)
        ...
        model = load_model_checkpoint(model, settings.CIF10RN34_SOTA_CKPT)
    ...
    elif args.net == 'cif10rn34':

        depth = 34
        widen_factor = 0
        model = resnet34(settings.MAX_CLASSES_CIF10, preprocessing=net_normalization)
        ckpt = torch.load(settings.CIF10RN34_CKPT)
        new_state_dict = create_new_state_dict(ckpt)
        model.load_state_dict(new_state_dict)
        ...
        depth = 34
        widen_factor = 0
        model = resnet34(settings.MAX_CLASSES_CIF100, preprocessing=net_normalization)
        ckpt = torch.load(settings.CIF100RN34_CKPT)
        new_state_dict = create_new_state_dict(ckpt)
        model.load_state_dict(new_state_dict)
        ...
        model = load_rb_model(model_name='Gowal2021Improving_28_10_ddpm_100m', dataset='cifar10', threat_model='Linf')
...
def load_test_set(args, preprocessing=None, num_workers=4, download=True, IS_TRAIN=False, clean_data=False, shuffle=True):
    ...
    if args.net == 'mnist':
        ...
        transform_list = [transforms.ToTensor(), transforms.Lambda( lambda x: x.repeat(3, 1, 1) ) ] + normalization
...
def create_output_filename(args, TRANSFER=None):
    if TRANSFER == None or TRANSFER == 'attack':
        appendix = get_appendix(args.num_classes, settings.MAX_CLASSES_IMAGENET)
    elif TRANSFER == 'data':
        appendix = get_appendix(args.num_classes_eval, settings.MAX_CLASSES_IMAGENET)
