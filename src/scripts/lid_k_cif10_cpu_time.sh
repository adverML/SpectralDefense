...
DATASETS="cif10 cif10vgg imagenet"
                            ...
                            for nrsamples in $NRSAMPLES; do
                                for classifier in $CLF; do
                                   ...
                                   if [ "$net" == imagenet ]; then
                                        python -u detect_adversarials.py --net "$net" --attack "$att" --detector "$det" --wanted_samples "$nrsamples" --clf "$classifier" --k_lid "$lidk" --eps "$eps" --num_classes 1000  --run_nr "$run"  --pca_features "$PCA_FEATURES"
                                    fi
