...
DETECTORS="LayerMFS LID Mahalanobis"
            ...
            for att in $ATTACKSLAYERNR; do
                for det in $DETECTORSLAYERNR; do
                    for nr in $LAYERNR; do 
                        ...
                        if [ "$net" == imagenet64 ]; then
                            python -u extract_characteristics.py --net "$net" --attack "$att" --detector "$det" --num_classes 1000 --nr "$nr" --run_nr "$run" --img_size 64   --wanted_samples "$WANTEDSAMPLES"  --take_inputimage_off
                        fi
                    done
                done
            done
