        ...
        for net in $DATASETS; do
            for att in $ATTACKS; do
                for eps in $EPSILONS; do
                    if [ "$net" == imagenet ]; then
                        if [ "$att" == std ]; then
                            python -u attacks.py --net "$net" --attack "$att" --img_size 32 --batch_size 128 --num_classes 1000 --wanted_samples_tr "$WANTEDSAMPLES_TR" --wanted_samples_te "$WANTEDSAMPLES_TE" --eps "$eps" --run_nr "$run"
                        else
                            python -u attacks.py --net "$net" --attack "$att" --img_size 32 --batch_size 500 --num_classes 1000 --wanted_samples_tr "$WANTEDSAMPLES_TR" --wanted_samples_te "$WANTEDSAMPLES_TE" --eps "$eps" --run_nr "$run"
                        fi
                    fi 
                done
            done
