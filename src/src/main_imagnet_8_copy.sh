    ...
    for run in $RUNS; do
        for net in $DATASETS; do
            if [ "$net" == imagenet ]; then
                python -u generate_clean_data.py --net "$net" --num_classes 1000 --wanted_samples 8000 --run_nr "$run" #--shuffle_off
            fi 
        done
    done
}
        ...
        for net in $DATASETS; do
            for att in $ATTACKS; do
                for eps in $EPSILONS; do
                    if [ "$net" == imagenet ]; then
                        if [ "$att" == std ]; then
                            python -u attacks.py --net "$net" --attack "$att" --img_size 32 --batch_size 128 --num_classes 1000 --wanted_samples $NRWANTEDSAMPLES --all_samples 8000 --eps "$eps" --run_nr "$run"
                        else
                            python -u attacks.py --net "$net" --attack "$att" --img_size 32 --batch_size 500 --num_classes 1000 --wanted_samples $NRWANTEDSAMPLES --all_samples 8000 --eps "$eps" --run_nr "$run"
                        fi
                    fi 
                done
            done
            ...
            for att in $ATTACKS; do  
                for eps in $EPSILONS; do
                    for det in $DETECTORS; do
                        python -u extract_characteristics.py --net "$net" --attack "$att" --detector "$det" --num_classes 1000 --run_nr "$run"  --eps "$eps" --wanted_samples $NRWANTEDSAMPLES --take_inputimage_off
                    done
                done
            done
        done
