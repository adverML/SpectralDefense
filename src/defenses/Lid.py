...
from defenses.Spectral import calculate_spectra, calculate_fourier_spectrum
...
def get_k(args):
    ...
    if args.k_lid < 0:
        if args.net == 'mnist' or args.net == 'cif10' or args.net == 'cif10vgg' or args.net == 'imagenet32' or args.net == 'celebaHQ32':
            k = 20 # original setup: https://arxiv.org/pdf/1801.02613.pdf
...
def calcFFTmultiLID(args, features):
    
    fft_features = []

    for feature in features:
        if args.detector == "FFTmultiLIDMFS":
            feature = calculate_fourier_spectrum(feature, typ='MFS')
        elif args.detector == "FFTmultiLIDPFS":
            feature = calculate_fourier_spectrum(feature, typ='PFS')
