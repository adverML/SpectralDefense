            ...
            for att in $ATTACKS; do  
                for eps in $EPSILONS; do
                    for det in $DETECTORS; do
                        if [ "$net" == celebaHQ32 ]; then
                            python -u extract_characteristics.py --net "$net"  --num_classes $NUMCLASSES --attack "$att" --detector "$det" --eps "$eps" --run_nr "$run"
                        fi 
                        if [ "$net" == celebaHQ64 ]; then
                            python -u extract_characteristics.py --net "$net"  --num_classes $NUMCLASSES --attack "$att" --detector "$det" --img_size 64 --eps "$eps" --run_nr "$run"
                        fi 
                        if [ "$net" == celebaHQ128 ]; then
                            python -u extract_characteristics.py --net "$net"  --num_classes $NUMCLASSES --attack "$att" --detector "$det" --img_size 128 --eps "$eps" --wanted_samples 1500 --run_nr "$run"
                        fi 

                        if [ "$net" == celebaHQ256 ]; then
                            python -u extract_characteristics.py --net "$net"  --num_classes $NUMCLASSES --attack "$att" --detector "$det" --img_size 256 --eps "$eps" --wanted_samples 1500 --run_nr "$run"
                        fi 
                    done
                done
            done
